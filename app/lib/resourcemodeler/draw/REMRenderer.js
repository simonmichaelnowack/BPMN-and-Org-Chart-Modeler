...
export default function REMRenderer(
  ...
  function drawRectWithBorder(parentGfx, width, height, r, offset, attrs) {
    if (isObject(offset)) {
      attrs = offset;
      offset = 0;
    }
    ...
    var borderOffset = 2.5;
    var rect2 = svgCreate("rect");
    svgAttr(rect2, {
      x: offset - borderOffset,
      y: offset - borderOffset,
      width: width + borderOffset * 2 - offset * 2,
      height: height + borderOffset * 2 - offset * 2,
      rx: r,
      ry: r,
      fill: "transparent",
      stroke: "black",
    ...
    svgAttr(rect1, {
      x: offset + borderOffset,
      y: offset + borderOffset,
      width: width - offset * 2 - borderOffset * 2,
      height: height - offset * 2 - borderOffset * 2,
      rx: r,
      ry: r,
    });
    svgAttr(rect1, attrs);
    svgAppend(parentGfx, rect1);
  ...
  function renderAttributes(parentGfx, element) {
    ...
    text += "\nAvailability: ";
    if (
      !isNaN(parseInt(semantic.availabilityStart, 10)) &&
      !isNaN(parseInt(semantic.availabilityEnd, 10))
    ) {
  ...
  this.handlers = {
    "rem:Position": function (parentGfx, element, attrs) {
      var rect = drawRect(
        parentGfx,
        element.width,
        element.height,
    ...
    "rem:OrganizationalUnit": function (parentGfx, element, attrs) {
      var rect = drawRectWithBorder(
        parentGfx,
        element.width,
        element.height,
        0,
