...
export default [
  {
    title:
      "TC2: Include all relevant data objects in the Termination Condition",
    id: "TC2",
        ...
        return [
          {
            ...
            message:
              "Please include at least one data object configuration as literal in the termination condition.",
          },
        ];
      } else {
        return [];
  ...
  },
  {
    ...
    id: "F9",
    getViolations(mediator) {
      const gateways = mediator.fragmentModelerHook.modeler
        .get("elementRegistry")
        .filter((element) => is(element, "bpmn:Gateway"));
      gateways.filter((gateway) => gateway.incoming.length === 0);
      return gateways
        .filter((gateway) => gateway.incoming.length === 0)
        .map((gateway) => ({
  ...
  },
  {
    ...
    id: "F3",
    getViolations(mediator) {
      const elements = mediator.fragmentModelerHook.modeler
        .get("elementRegistry")
        .filter(
          (element) =>
      ...
      const activities = mediator.fragmentModelerHook.modeler
        .get("elementRegistry")
        .filter((element) => is(element, "bpmn:Activity"));
      for (const activity of activities) {
        if (connectedElements.has(activity)) {
          continue;
        }
        getConnectedElements(activity).forEach((element) =>
          connectedElements.add(element.id)
        );
      }
      return elements
        .filter((element) => !connectedElements.has(element.id))
        .map((element) => ({
          element: element.businessObject,
          message:
            "Please connect this " +
            type(element.businessObject) +
            " to a component with an activity.",
        }));
    },
    severity: SEVERITY.ERROR,
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const dataObjects = fragmentModeler
        .get("elementRegistry")
        .filter(
          (element) =>
            is(element, "bpmn:DataObjectReference") &&
      ...
      return dataObjects
        .filter(
          (dataObject) => !dataObject.states || dataObject.states.length === 0
        )
        .map((dataObject) => ({
          element: dataObject,
          message:
            "Please provide at least one state for data object references.",
        }));
    },
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const clazzes = dataModeler
        .get("elementRegistry")
        .getAll()
        .filter((element) => is(element, "od:Class"));
      return clazzes
        .filter((element) => element.businessObject.attributeValues)
        .map((clazz) => ({
          element: clazz.businessObject,
          message:
            "Attributes are only used very rarely. Consider using states instead.",
        }));
    },
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      return activities
        .filter((element) => !activites_with_data.includes(element))
        .map((element) => ({
          element: element.businessObject,
          message:
            "Consider using a data object to model a pre- and postcondition for this activity.",
        }));
    },
    severity: SEVERITY.WARNING,
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const startEvents = hook.modeler
        .get("elementRegistry")
        .filter(
          (element) =>
            is(element, "bpmn:StartEvent") && element.type !== "label"
        );
        ...
        return [
          {
            element: hook.getRootObject(),
            message:
              "Please use at least one start event in the fragment model.",
          },
        ];
      } else {
        return [];
    ...
    getViolations(mediator) {
      ...
      if (startEvents.length > 1) {
        return startEvents.map((element) => ({
          element: element.businessObject,
          message:
            "Your process has multiple start events. Please ensure that this is intended.",
        }));
      } else {
        return [];
      }
  ...
  },
  {
    ...
    getViolations(mediator) {
      const dataModeler = mediator.dataModelerHook.modeler;
      const fragmentModeler = mediator.fragmentModelerHook.modeler;
      const startEvents = fragmentModeler
        .get("elementRegistry")
        .filter(
          (element) =>
            is(element, "bpmn:StartEvent") && element.type !== "label"
        );
      const caseClasses = dataModeler
        .get("elementRegistry")
        .filter((element) => is(element, "od:Class"))
        .map((element) => element.businessObject)
      ...
      return startEvents.flatMap((startEvent) => {
        ...
        const createdClasses = connectedElements
          .filter((element) => is(element, "bpmn:DataObjectReference"))
          .map((element) => element.businessObject.dataclass);
        const caseClassConnected = createdClasses.some((createdClass) =>
          caseClasses.includes(createdClass)
        );
          ...
          return [
            {
              ...
              message:
                "Please make one of the nodes of this start fragment create an object of one of the case classes: " +
                caseClasses.map((clazz) => '"' + clazz.name + '"').join(", "),
              quickFixes: caseClasses.flatMap((caseClass) =>
                connectedElements
                  .filter(
                    (element) =>
                      is(element, "bpmn:Activity") ||
                  ...
                  .map((element) => ({
                    label:
                      'Add data object reference that writes case class "' +
                      caseClass.name +
                      '" to node "' +
                      element.businessObject.name +
                      '"',
                    action: (event) =>
                      fragmentModeler.startDoCreation(
                        event,
                        element,
                        caseClass
                      ),
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const activities = fragmentModeler
        .get("elementRegistry")
        .filter((element) => is(element, "bpmn:Activity"));

      return activities
        .map((activity) => activity.businessObject)
        .flatMap((activity) => {
          // TODO improve after introduction of IO-Sets
          const statesPerClass = {};
          activity.dataInputAssociations
            ?.map((assoc) => assoc.sourceRef[0])
            .filter(
              (dataObjectReference) =>
                dataObjectReference.dataclass && dataObjectReference.states
            )
            .forEach((dataObjectReference) => {
              ...
              statesPerClass[dataObjectReference.dataclass.id].incoming.push(
                ...dataObjectReference.states
              );
            });
          activity.dataOutputAssociations
            ?.map((assoc) => assoc.targetRef)
            .filter(
              (dataObjectReference) =>
                dataObjectReference.dataclass && dataObjectReference.states
            )
            .forEach((dataObjectReference) => {
              ...
              statesPerClass[dataObjectReference.dataclass.id].outgoing.push(
                ...dataObjectReference.states
              );
            });
          const uncovered = Object.keys(statesPerClass)
            .map((classId) => {
              const olc = olcModeler.getOlcByClass(
                statesPerClass[classId].clazz
              );
              const transitionsInOlc = olc
                .get("Elements")
                .filter((element) => is(element, "olc:Transition"));

              function transitionsMatching(sourceState, targetState) {
                return transitionsInOlc.filter(
                  (transition) =>
              ...
              const uncoveredInputstates = statesPerClass[
                classId
              ].incoming.filter((sourceState) => {
                // Has target states of same class and none is covered
                return (
                  statesPerClass[classId].outgoing.length > 0 &&
                  statesPerClass[classId].outgoing.filter((targetState) => {
                    return (
                      sourceState === targetState ||
                      transitionsMatching(sourceState, targetState).length > 0
                    );
                  }).length === 0
                );
              });

              const uncoveredOutputstates = statesPerClass[
                classId
              ].outgoing.filter((targetState) => {
                // Has source states of same class and none is covered
                return (
                  statesPerClass[classId].incoming.length > 0 &&
                  statesPerClass[classId].incoming.filter((sourceState) => {
                    return (
                      sourceState === targetState ||
                      transitionsMatching(sourceState, targetState).length > 0
                    );
            ...
            .filter(
              ({ uncoveredInputstates, uncoveredOutputstates }) =>
                uncoveredInputstates.length + uncoveredOutputstates.length > 0
            );

          function stringifyUncovered({
            uncoveredInputstates,
            uncoveredOutputstates,
          }) {
            return (
              "" +
              ((uncoveredInputstates.length > 0 &&
                "input state(s) " +
                  uncoveredInputstates
                ...
                "") +
              ((uncoveredInputstates.length > 0 &&
                uncoveredOutputstates.length > 0 &&
                " and ") ||
                "") +
              ((uncoveredOutputstates.length > 0 &&
                "output state(s) " +
                ...
                "") +
              ' of class "' +
              (uncoveredInputstates[0] || uncoveredOutputstates[0])?.$parent
                .name +
              '"'
            );
          }
          ...
          function transitionToQuickFix(sourceState, targetState) {
            return {
              label:
                "Create transition: " +
                stringifyTransition(sourceState, targetState) +
                " in OLC " +
                sourceState.$parent.name,
              action: () =>
                mediator.olcModelerHook.modeler.createTransition(
                  sourceState,
                  targetState
                ),
            };
            ...
            return [
              {
                ...
                message:
                  "Please cover the " +
                  uncovered.map(stringifyUncovered).join(", ") +
                  " with valid transitions in the OLC.",
                quickFixes: uncovered.flatMap(
                  ({ classId, uncoveredInputstates, uncoveredOutputstates }) =>
                    [
                      ...uncoveredInputstates.flatMap((sourceState) =>
                        statesPerClass[classId].outgoing.map((targetState) => ({
                          sourceState,
                          targetState,
                        }))
                      ),
                      ...uncoveredOutputstates.flatMap((targetState) =>
                        statesPerClass[classId].incoming
                          .filter(
                            (sourceState) =>
                              !uncoveredInputstates.includes(sourceState)
                          )
                          .map((sourceState) => ({
                            sourceState,
                            targetState,
                    ...
                    ].map(({ sourceState, targetState }) =>
                      transitionToQuickFix(sourceState, targetState)
                    )
                ),
              },
            ];
    ...
    getViolations(mediator) {
      ...
      return activities.flatMap((activity) => {
        ...
        const writtenClasses = new Set(
          activity.dataOutputAssociations
            ?.filter((assoc) => assoc.targetRef.dataclass)
            .map((assoc) => assoc.targetRef.dataclass)
        );
        const readClasses = new Set(
          activity.dataInputAssociations?.map(
            (assoc) => assoc.sourceRef[0].dataclass
          )
        );
        const createdClasses = [...writtenClasses].filter(
          (clazz) => !readClasses.has(clazz)
        );

        const missingAssociations = createdClasses.flatMap((createdClass) => {
          return (classDependencies[createdClass.id] || [])
            .filter(
              (contextClass) =>
                !writtenClasses.has(contextClass) &&
                !readClasses.has(contextClass)
            )
            .map((contextClass) => ({
              createdClass,
              contextClass,
        ...
        function stringifyMissing({ createdClass, contextClass }) {
          return (
            'to "' + contextClass.name + '" for "' + createdClass.name + '"'
          );
        }

        if (missingAssociations.length > 0) {
          const activityShape = fragmentModeler
            .get("elementRegistry")
            .get(activity.id);
          return [
            {
              ...
              message:
                'In activity "' +
                activity.name +
                '", please add references to the following context classes: ' +
                missingAssociations.map(stringifyMissing).join(", "),
              quickFixes: missingAssociations.flatMap(({ contextClass }) => [
                {
                  label:
                    'Add reading data object reference of class "' +
                    contextClass.name +
                    '" to activity "' +
                    activity.name +
                    '"',
                ...
                {
                  label:
                    'Add writing data object reference of class "' +
                    contextClass.name +
                    '" to activity "' +
                    activity.name +
                    '"',
        ...
        return [
          {
            ...
            quickFixes: clazzes.map((clazz) => ({
              label:
                'Make class "' + clazz.businessObject.name + '" a case class',
              action: (action) =>
                dataModeler.updateProperty(clazz, { caseClass: true }),
            })),
          },
        ];
      } else {
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const connectedWithCaseClass = getConnectedByExistentialAssociation(
        caseClasses[0],
        classDependents
      );
      const notConnectedClasses = clazzes.filter(
        (clazz) => connectedWithCaseClass.indexOf(clazz) === -1
      );

      return notConnectedClasses.map((clazz) => ({
        element: clazz,
        message:
          'Please connect class "' +
          clazz.name +
          '" to the case class via a path of existential associations.',
      }));
    },
    severity: SEVERITY.ERROR,
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      function dfs(element) {
        ...
        const outgoingDependencies = element.outgoing.filter(
          (connection) => connection.type === "dep:Dependency"
        );
      ...
      const startState = elements.find(
        (element) => element.id === "start_state"
      );
      dfs(startState);
  ...
  },
  {
    ...
    getViolations(mediator) {
      const dependencyModeler = mediator.dependencyModelerHook.modeler;
      const dependencies = dependencyModeler
        .get("elementRegistry")
        .getAll()
        .filter((element) => is(element, "dep:Dependency"));
      const dependenciesFromStartState = dependencies.filter(
        (element) => element.source.id === "start_state"
      );
  ...
  },
  {
    ...
    getViolations(mediator) {
      ...
      const elementRegistry = roleModeler.get("elementRegistry");
